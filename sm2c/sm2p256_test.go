...
func TestSM2P256Mult(t *testing.T) {
	p256 := SM2P256()
...
func TestP256CombinedMult(t *testing.T) {
	...
	p256, ok := SM2P256().(combinedMult)
	if !ok {
		p256 = &synthCombinedMult{SM2P256()}
	}
...
func TestIssue52075(t *testing.T) {
	Gx, Gy := SM2P256().Params().Gx, SM2P256().Params().Gy
	scalar := make([]byte, 33)
	scalar[32] = 1
	x, y := SM2P256().ScalarBaseMult(scalar)
	if x.Cmp(Gx) != 0 || y.Cmp(Gy) != 0 {
